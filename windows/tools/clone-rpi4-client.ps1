...
function Normalize-DeviceId {
    ...
    if ($clean -notmatch "^[a-z0-9][a-z0-9-]{1,61}[a-z0-9]$") {
        throw "Device ID는 hostname으로 쓸 수 있는 값이어야 합니다. 예: rpi-001"
    }
    return $clean
}
...
function Set-RootfsIdentity {
    ...
    $cloudCfg = Join-Path $etc "cloud\cloud.cfg.d"
    if (Test-Path -LiteralPath (Join-Path $etc "cloud")) {
        New-Item -ItemType Directory -Force -Path $cloudCfg | Out-Null
        $cloudIdentity = @(
            "# Generated by RPI Netboot Manager",
            "preserve_hostname: true",
            "manage_etc_hosts: false"
        )
        Set-Content -LiteralPath (Join-Path $cloudCfg "99-rpi-netboot-identity.cfg") -Value $cloudIdentity -Encoding ASCII
    }

    $metadataDir = Join-Path $etc "rpi-netboot"
    New-Item -ItemType Directory -Force -Path $metadataDir | Out-Null
    $metadata = [ordered]@{
        device_id = [string]$Client.hostname
        hostname = [string]$Client.hostname
        serial = [string]$Client.serial
        mac = [string]$Client.mac
        ip = [string]$Client.ip
        server_ip = [string]$ConfigObject.server_ip
        router_ip = [string]$ConfigObject.router_ip
        nfs_alias = [string]$ConfigObject.nfs_alias
        role = "rpi4-netboot-client"
        zero2w_gadget_peer_hint = "10.73.0.11"
        generated_at = (Get-Date).ToString("o")
    }
    Set-Content -LiteralPath (Join-Path $metadataDir "client.json") -Value ($metadata | ConvertTo-Json -Depth 6) -Encoding UTF8
    $envLines = @(
        "RPI_DEVICE_ID=$($Client.hostname)",
        "RPI_SERIAL=$($Client.serial)",
        "RPI_MAC=$($Client.mac)",
        "RPI_IP=$($Client.ip)",
        "RPI_NETBOOT_SERVER=$($ConfigObject.server_ip)",
        "RPI_ZERO2W_GADGET_PEER_HINT=10.73.0.11"
    )
    Set-Content -LiteralPath (Join-Path $metadataDir "client.env") -Value $envLines -Encoding ASCII

    Write-Check "정상" "rootfs identity" "$root hostname/machine-id/SSH host keys/client metadata reset"
}
...
function New-ClientRecord {
    ...
    $newDeviceId = Normalize-DeviceId $DeviceId
    $newHostname = if (-not [string]::IsNullOrWhiteSpace($Hostname)) {
        Normalize-DeviceId $Hostname
    } elseif (-not [string]::IsNullOrWhiteSpace($newDeviceId)) {
        $newDeviceId
    } else {
        $newSerial
    }
...
function Show-Plan {
    ...
    Write-Output "   Optional: -DeviceId rpi-001 sets hostname and /etc/rpi-netboot/client.json"
...
function Verify-Client {
    ...
    $metadataPath = Join-Path $root "etc\rpi-netboot\client.json"
    ...
    Write-Check $(if ($hasMetadata) { "정상" } else { "주의" }) "client metadata" $metadataPath
