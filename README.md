...
- **SSH 자동 활성화**: 모든 클라이언트에 SSH 자동 활성화 및 설정
- **sudo 권한 자동 수정**: NFS 환경에서 sudo 정상 작동 보장
...
for ip in {100..105}; do
    echo -n "192.168.0.$ip: "
    timeout 1 nc -zv 192.168.0.$ip 22 2>&1 | grep -o "succeeded" || echo "닫힘"
done
```
...
인터넷
  ...
  ├── 공유기 (192.168.0.1)
  │     │
  │     ├── PXE 서버 (192.168.0.10) [SSH: 22]
  │     │
  │     ├── RPi 클라이언트 #1 (192.168.0.100) [SSH: 22]
  │     ├── RPi 클라이언트 #2 (192.168.0.101) [SSH: 22]
  │     ├── RPi 클라이언트 #3 (192.168.0.102) [SSH: 22]
  │     └── ... 
```
